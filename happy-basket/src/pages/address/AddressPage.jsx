...
function AddressPage() {
  ...
  const { addresses, mobile } = state.currentUser;
  const isUserLoggedIn = mobile;

  return (
    <>
      {isUserLoggedIn && (
        <div className="bg-black/80 min-h-vh pb-3">
          {/* Back button */}
          ...
          {!page ? (
            // For search purpose
            <div className=" relative flex flex-col w-[95%] mx-auto mt-3 px-3 py-4 min-h-125 rounded-xl bg-white">
              ...
              <div className="flex flex-col">
                {addresses && <AddressCard addresses={addresses} />}
              </div>
            </div>
          ) : (
